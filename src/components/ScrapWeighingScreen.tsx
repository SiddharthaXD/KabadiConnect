...
export const ScrapWeighingScreen: React.FC<ScrapWeighingScreenProps> = ({
  ...
  const [selectedRecyclerId, setSelectedRecyclerId] = useState<string>(
    recyclers.find((r) => r.isOpen !== false)?.id || recyclers[0]?.id || 'rec-1'
  );

  const selectedRecycler =
    recyclers.find((r) => r.id === selectedRecyclerId) || recyclers[0] || RECYCLERS[0];
  const isSelectedRecyclerOpen = selectedRecycler.isOpen !== false;
  ...
  const handleToggleRecycler = (rec: Recycler) => {
    ...
    if (!isOpen) {
      speakVernacular(
        language === 'mr'
          ? `${rec.nameMr || rec.nameHi} सध्या बंद आहे.`
          : language === 'en'
          ? `${rec.nameEn} is currently closed.`
          : `${rec.nameHi} वर्तमान में बंद है।`,
        language
      );
    } else {
      speakVernacular(
  ...
  const handleVoiceWeightHelp = () => {
    ...
    speakVernacular(
      language === 'mr'
        ...
        : 'कांटे का सटीक वजन भरें या प्रीसेट बटन दबाएं। नीचे कुल रकम दिखाई देगी।',
      language
    );
  };

  const handleConfirm = () => {
    if (!isSelectedRecyclerOpen) {
      triggerHaptic([100, 50, 100]);
      speakVernacular(
        language === 'mr'
          ? 'हे केंद्र सध्या बंद आहे. कृपया सुरू असलेले अधिकृत केंद्र निवडा.'
          : language === 'en'
          ? 'This center is currently closed. Please choose an open certified recycler.'
          : 'यह रीसाइक्लिंग केंद्र वर्तमान में बंद है। कृपया खुला अधिकृत केंद्र चुनें।',
        language
      );
      return;
    }
    ...
    speakVernacular(
      language === 'hi'
        ...
        : language === 'mr'
        ? `सौदा यशस्वी! ₹${totalPayout.toLocaleString('en-IN')} चे टोकन तयार केले आहे.`
  ...
  return (
    <div className="flex flex-col w-full max-w-md mx-auto px-4 pt-20 pb-36 gap-3">
      ...
      <div className="flex items-center justify-between px-1 pt-1">
        ...
        <span className="text-[12px] font-bold text-[#565e74]">
          {recyclers.length} {language === 'mr' ? 'खरेदीदार' : language === 'en' ? 'Buyers' : 'खरीदार'}
        </span>
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div
          id="card-greentech"
          className={`flex flex-col rounded-xl bg-white shadow-md overflow-hidden border-3 transition-all ${
            isSelectedRecyclerOpen
              ? 'border-[#006948]'
              : 'border-[#ba1a1a]'
          }`}
        >
          ...
          <div className="p-3.5 bg-white flex flex-col gap-1.5">
            <div className="flex items-start justify-between gap-2">
              <div className="flex flex-col flex-1 min-w-0">
                <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                  ...
                  <span
                    className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-black uppercase shadow-xs ${
                      isSelectedRecyclerOpen
                        ? 'bg-[#85f8c4] text-[#002114]'
                        : 'bg-[#ffdad6] text-[#ba1a1a]'
                    }`}
                  >
                    <span className={`w-1.5 h-1.5 rounded-full ${isSelectedRecyclerOpen ? 'bg-[#006948] animate-pulse' : 'bg-[#ba1a1a]'}`} />
                    {isSelectedRecyclerOpen
                      ? (t.centerStatusOpen || 'OPEN')
                      : (t.centerStatusClosed || 'CLOSED')}
                  </span>
                </div>
            ...
            {/* Warning if selected recycler is closed */}
            {!isSelectedRecyclerOpen && (
              <div className="mt-1 p-2.5 bg-[#ffdad6]/50 border border-[#ba1a1a]/40 rounded-lg flex items-start gap-2 text-[#410002]">
                <span className="material-symbols-outlined text-[18px] text-[#ba1a1a] shrink-0 mt-0.5">warning</span>
                <div className="flex flex-col">
                  <span className="text-[12px] font-bold leading-tight">
                    {t.centerClosedWarning}
                  </span>
                  <span className="text-[10px] opacity-85 mt-0.5">
                    {language === 'mr'
                      ? 'हे केंद्र सध्या बंद आहे. कृपया खाली सुरू असलेले केंद्र निवडा.'
                      : language === 'en'
                      ? 'This center is currently closed. Please select an open buyer below.'
                      : 'यह केंद्र वर्तमान में बंद है। कृपया नीचे खुला खरीदार चुनें।'}
                  </span>
                </div>
              </div>
            )}
        ...
        {recyclers.filter((r) => r.id !== selectedRecycler.id).map((altRecycler) => {
          const isAltOpen = altRecycler.isOpen !== false;
          return (
            <div
              key={altRecycler.id}
              onClick={() => handleToggleRecycler(altRecycler)}
              className={`flex flex-col rounded-xl bg-white shadow-sm p-3.5 transition-all cursor-pointer border-2 ${
                isAltOpen
                  ? 'border-[#bccac0]/60 hover:border-[#006948] opacity-95 hover:opacity-100'
                  : 'border-[#ffdad6] bg-[#fff8f8] opacity-80 hover:opacity-95'
              }`}
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex flex-col flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                    ...
                    >
                      <span className={`w-1.5 h-1.5 rounded-full ${isAltOpen ? 'bg-[#006948]' : 'bg-[#ba1a1a]'}`} />
                      {isAltOpen
                        ? (t.centerStatusOpen || 'OPEN')
                        : (t.centerStatusClosed || 'CLOSED')}
                    </span>
      ...
      <div className="fixed bottom-0 inset-x-0 bg-white/95 backdrop-blur-md p-3 pb-safe shadow-[0_-4px_16px_rgba(0,0,0,0.08)] z-40 border-t border-[#bccac0]/40">
        <div className="flex flex-col gap-1.5 max-w-md mx-auto w-full">
          ...
          <button
            ...
            className={`h-15 w-full rounded-xl font-bold text-[17px] flex items-center justify-between px-4 active:scale-[0.98] transition-transform shadow-md border-2 cursor-pointer ${
              isSelectedRecyclerOpen
                ? 'bg-[#006948] hover:bg-[#005a3e] text-white border-[#002114]'
                : 'bg-[#ba1a1a] hover:bg-[#93000a] text-white border-[#410002]'
            }`}
          >
            <span className="flex items-center gap-2">
              <span className="material-symbols-outlined text-[24px]">
                {isSelectedRecyclerOpen ? 'handshake' : 'lock'}
              </span>
              <span className="truncate">
                {isSelectedRecyclerOpen
                  ? t.confirmHandover
                  : (language === 'mr' ? 'केंद्र बंद आहे (Center Closed)' : language === 'en' ? 'Center Closed - Select Open Buyer' : 'केंद्र बंद है - खुला खरीदार चुनें')}
              </span>
            </span>
            <div className="flex items-center gap-1">
              ...
              <span className="material-symbols-outlined text-[22px]">
                {isSelectedRecyclerOpen ? 'arrow_forward' : 'block'}
              </span>
            </div>
          </button>
        </div>
